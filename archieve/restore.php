...
// Update the status of the product and set to active, so that it will be removed from the archived
$connection->query("UPDATE product SET status = 'active' WHERE token = '$id'");
$productName = htmlspecialchars($row[0]["product_name"]);
$productPrice = htmlspecialchars($row[0]["price"]);
echo "<script>alert('The Product item " . $productName . " " . $productPrice . " is successfully restored.');</script>";
