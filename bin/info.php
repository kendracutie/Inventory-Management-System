...
// Check if the info session is set
...
<h2 style="color: white">Recycle Bin</h2>
...
<button class="btn btn-secondary" onclick="window.location = '../archieve/info.php';"><i class="fa fa-wrench" aria-hidden="true" style="color: grey"></i>View Archieved</button>
...
<table class="table text-dark text-center table-striped table-hover table-bordered">
    ...
    // Get all the data in the product table except for those data that are in the bin
    $result = $connection->query("SELECT * FROM product WHERE status = 'bin'");
        ...
        foreach ($result as $row) {
            ...
            <tr>
                ...
                <td>
                    ...
                    <a href="./delete.php?id=<?php echo htmlspecialchars($row['token']); ?>" onclick="return confirmDelete()"><i class="fas fa-trash" style="color: red" title="Delete"></i></a>
                </td>
            </tr>
            <?php
        }
...
<script>
    ...
    function confirmLogout() {
        return confirm("This will log you out from this application.\nDo you still want to continue?");
    }

    // Function that will be called if Archieved link is clicked
    function confirmArchieved() {
        return confirm("This link will archive the selected data. After that, you are still able to restore it.\n\n Do you still want to continue?");
    }

    // Function that will be called if Restore link is clicked
    function confirmRestore() {
        return confirm("This link will restore the selected data and will be removed from the bin.\n\n Do you still want to continue?");
    }

    // Function that will be called if data is subject for permanent delete
    function confirmDelete() {
        return confirm("This link will permanently delete the selected data. After that, you will no longer be able to restore it.\n\n Do you still want to continue?");
    }
</script>
</html>
